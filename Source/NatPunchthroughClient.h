...
struct RAK_DLL_EXPORT PunchthroughConfiguration
{
	...
	PunchthroughConfiguration() {
		TIME_BETWEEN_PUNCH_ATTEMPTS_INTERNAL=100;
		TIME_BETWEEN_PUNCH_ATTEMPTS_EXTERNAL=200;
		UDP_SENDS_PER_PORT_INTERNAL=1;//3
		UDP_SENDS_PER_PORT_EXTERNAL=5;//8
		INTERNAL_IP_WAIT_AFTER_ATTEMPTS=100;
        MAXIMUM_NUMBER_OF_INTERNAL_IDS_TO_CHECK = 0; /// set to 0 to not do lan connects
		MAX_PREDICTIVE_PORT_RANGE=2;
		EXTERNAL_IP_WAIT_BETWEEN_PORTS=200;
		EXTERNAL_IP_WAIT_AFTER_FIRST_TTL=200;
